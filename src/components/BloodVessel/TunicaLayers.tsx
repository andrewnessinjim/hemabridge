...
type Props = {
  ...
  grayscale?: boolean;
  /** Renders one fixed frame to an offscreen canvas and blits that every
   * frame, instead of regenerating the noise wave and replaying every
   * layer/texture draw call 60 times a second. Cheap to keep on-screen,
   * but the wave stops animating. */
  static?: boolean;
};
...
export default function TunicaLayers({
  ...
  const offscreenSizeRef = React.useRef<{ width: number; height: number } | null>(null);
  ...
  React.useEffect(() => {
    ...
    function drawFrame(
      ...
      function drawLayer(
        ...
        segments.forEach((segmentPoints) => {
          ...
          targetCtx.moveTo(firstPoint.x, firstPoint.y + yOffset);
          for (const point of restPoints) {
            targetCtx.lineTo(point.x, point.y + yOffset);
          }
          targetCtx.strokeStyle = layer.color;
          targetCtx.lineWidth = layer.height;
    ...
    const unregister = register(
      ({ ctx, dpr, canvasWidth, canvasHeight, totalTime }) => {
        if (!staticMode) {
          drawFrame(ctx, canvasWidth, canvasHeight, totalTime);
          return;
        }

        const sizeChanged =
          !offscreenCanvasRef.current ||
          offscreenSizeRef.current?.width !== canvasWidth ||
          offscreenSizeRef.current?.height !== canvasHeight;

        if (sizeChanged) {
          const offscreen = offscreenCanvasRef.current ?? document.createElement("canvas");
          offscreen.width = Math.max(1, Math.round(canvasWidth * dpr));
          offscreen.height = Math.max(1, Math.round(canvasHeight * dpr));

          const offscreenCtx = offscreen.getContext("2d");
          if (offscreenCtx) {
            offscreenCtx.setTransform(dpr, 0, 0, dpr, 0, 0);
            // Fixed totalTime=0: the noise wave and every draw call only
            // run once, then get reused as-is on every subsequent frame.
            drawFrame(offscreenCtx, canvasWidth, canvasHeight, 0);
          }

          offscreenCanvasRef.current = offscreen;
          offscreenSizeRef.current = { width: canvasWidth, height: canvasHeight };
        }

        if (offscreenCanvasRef.current) {
          ctx.drawImage(offscreenCanvasRef.current, 0, 0, canvasWidth, canvasHeight);
        }
      },
    );

    return unregister;
  }, [canvasContextValue, ratios, isClotting, mode, grayscale, staticMode]);
